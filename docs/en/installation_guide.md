...
- Using Kubernetes pods
    ...
    3. Applies to scenarios where high O&M automation is required. This deployment mode allows for centrally creating, managing, and deleting WAAS Booster service pods on master nodes, improving system reliability and availability.
...
>Master nodes refer to the management nodes in a Kubernetes cluster, and service nodes refer to the nodes deployed with specific services in a Kubernetes cluster.
...
[**Table 1**](#os-and-software-requirements) lists the OS and software requirements.

**Table 1** OS and software requirements<a id="os-and-software-requirements"></a>

|Item|Version|How to Obtain|
|--|--|--|
|OS|openEuler 20.03 LTS SP3<br>openEuler 22.03 LTS SP4|openEuler 20.03 LTS SP3: [Link](https://www.openeuler.org/en/download/archive/detail/?version=openEuler%2020.03%20LTS%20SP3)<br>openEuler 22.03 LTS SP4: [Link](https://www.openeuler.org/en/download/archive/detail/?version=openEuler%2022.03%20LTS%20SP4)|
...
8. <a id="li3962559112"></a>Check the `waasbooster_env` environment path.
    ...
    >![](public_sys-resources/icon-note.gif) **NOTE:**
    >-   If the `waasbooster_env` path in the command output in step [8](#li3962559112) is not `/usr/local/waasbooster/bin/python3`, copy the path for subsequent configuration file modification after WAAS Booster is deployed.
    >-   If the returned content is `/usr/local/waasbooster/bin/python3`, skip the modification.
...
10. Obtain the WAAS Booster open-source package `waas-waasbooster.zip`. For details about how to obtain the package, see [OS and software requirements](#os-and-software-requirements). Upload `waas-waasbooster.zip` to a custom directory on the server and decompress the package.
...
>-   After the uninstallation is complete, you are advised to manually delete the installation logs located in `/var/log/waasbooster.log` and installation packages.
...
10. Create and go to the WAAS Booster RPM compilation path, for example, `/home/waasbooster_rpm_build`.
    1. Create a path.
...
12. Obtain and decompress the WAAS software installation package `waas-waasbooster.zip`.
    ...
    For details about how to obtain the package, see [OS and software requirements](#os-and-software-requirements).
...
16. Create files and save the specified content.
    ...
    2. Create a `waas_booster.spec` file and add the following content to the file:
...
2. Download the WAAS Booster open-source package. For details, see [OS and software requirements](#os-and-software-requirements).
...
### Deploying a Pod<a name="EN-US_TOPIC_0000002518224410"></a>
...
### (Optional) Deleting a Pod<a name="EN-US_TOPIC_0000002518224404"></a>

To stop the WAAS Booster service in a cluster, you need to delete the pod. After the pod is deleted, the WAAS Booster service automatically stops, and the quota of all service pods is restored to the initial setting.
...
1. View pods.
    ...
    If the following information is displayed, the `waasbooster` pod is running.
...
2. Delete the pod.
    ...
    If the following information is displayed, the pod is deleted successfully.
